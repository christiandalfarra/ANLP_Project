...
A custom-built corpus of 99 football matches with audio commentary transcripts, sentence-level segments with prosodic features, and GPT-4-generated reference summaries (prompted per match identifier, spot-checked manually). Used as both training and evaluation data for the ANLP project.
...
A short match report of typically 600–2300 characters (~150–500 words), generated by prompting GPT-4 with the match identifier (year, competition, teams) and a fixed output template, then spot-checked manually. Structured as:
