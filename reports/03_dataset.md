...
A custom-built corpus of 99 football matches with audio commentary transcripts, sentence-level segments with prosodic features, and GPT-4-generated reference summaries (GPT-4 via ChatGPT web with search enabled, prompted per match identifier; grounded in web-retrieved match reports, with BBC sourcing verified for every match). Used as both training and evaluation data for the ANLP project.
...
A short match report of typically 600–2300 characters (~150–500 words), generated by prompting GPT-4 with the match identifier (year, competition, teams) and a fixed output template, via the ChatGPT web interface with search enabled, so facts are drawn from retrieved contemporary reports rather than parametric memory; the citations were checked for every match and each drew on the BBC website. Note retrieval-based generation is not exactly reproducible — search results change over time. Structured as:
...
- **End-to-end pipeline**: raw audio → Whisper transcripts → segment-aligned timed text → GPT-4 reference summaries. Reproducible from source.
